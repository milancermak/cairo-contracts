...
from utils import (
    ...
    MAX_UINT256, MAX_FELT, ZERO_ADDRESS, INVALID_UINT256, get_contract_def,
...
def contract_defs():
    ...
    erc20_def = get_contract_def(
        'openzeppelin/token/erc20/ERC20_Mintable_felt.cairo')
...
async def test_mint_overflow(token_factory):
    ...
    tx = await signer.send_transaction(
        account, erc20.contract_address, 'mint', [
            RECIPIENT,
            *(to_uint(MAX_FELT - 1000))
            #*pass_amount
        ])
    print(tx.call_info.execution_resources)
    # ExecutionResources(n_steps=705, builtin_instance_counter={'pedersen_builtin': 2, 'range_check_builtin': 23, 'ecdsa_builtin': 1, 'output_builtin': 0, 'bitwise_builtin': 0, 'ec_op_builtin': 0}, n_memory_holes=26)
    # ExecutionResources(n_steps=648, builtin_instance_counter={'pedersen_builtin': 2, 'range_check_builtin': 17, 'ecdsa_builtin': 1, 'output_builtin': 0, 'bitwise_builtin': 0, 'ec_op_builtin': 0}, n_memory_holes=26)
    # steps = 705 - 648 = 57
    # range_check_builtin = 23 - 17 = 8
